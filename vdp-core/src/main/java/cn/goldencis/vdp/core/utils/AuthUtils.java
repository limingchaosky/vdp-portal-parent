...
public class AuthUtils {
    public static Map<String, Object> getAuthInfo(ServletContext servletContext) {
        ...
        String deviceUnique = servletContext.getAttribute("deviceUnique") != null ? servletContext.getAttribute(
                "deviceUnique").toString() : "";
        ...
        if (ConstantsDto.VALIDATE_FLAG == 1) {
            if (f.isFile() && f.exists()) {
                ...
                try {
                    ...
                    //时间要么没字段 要么肯定有值
                    beginDate = !jasonObject.containsKey("startdate") ? "" : sdf.format(sdf.parse(jasonObject.get(
                            "startdate").toString()));
                    endDate = !jasonObject.containsKey("enddate") ? ConstantsDto.LONG_TIME_LIMIT : sdf.format(sdf
                            .parse(jasonObject.get("enddate").toString()));
                    supportDate = !jasonObject.containsKey("maintaindate") ? ConstantsDto.LONG_TIME_LIMIT : sdf
                            .format(sdf.parse(jasonObject.get("maintaindate").toString()));
                    String omsFlag = jasonObject.get("product").toString();

                    authInfo.put("company", company);
                    authInfo.put("beginEndDate", !ConstantsDto.LONG_TIME_LIMIT.equals(endDate) ? beginDate + " 到 "
                            + endDate : endDate);
                    ...
                    if ("".equals(deviceUnique) || !deviceUnique.equals(jasonObject.get("hwserial").toString())) {
                        ...
                        authmsg = "不在授权期限内";
                    } else if (!ConstantsDto.PROCJECT_IDENTIFICATION.equalsIgnoreCase(omsFlag)) {
                        authmsg = "项目标识不正确";
                    ...
                    //提示信息
                    if (!ConstantsDto.LONG_TIME_LIMIT.equals(supportDate)
                            && nowDate.after(DateUtil.strToDate(DateUtil.getDateAdd(-1, supportDate, "yyyy-MM-dd"),
                                    "yyyy-MM-dd"))) {
                        authInfo.put("promptMsg", "维保期限即将到期，请续保。");
                    }
                    //判断是否维保期限超期
                    if (!ConstantsDto.LONG_TIME_LIMIT.equals(supportDate)
                            && nowDate.after(DateUtil.strToDate(DateUtil.getDateAddDay(1, supportDate, "yyyy-MM-dd"),
                                    "yyyy-MM-dd"))) {

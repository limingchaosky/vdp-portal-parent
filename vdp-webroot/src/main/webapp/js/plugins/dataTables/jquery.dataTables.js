...
(function( $, window, document, undefined ) {
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		for ( var i=0 ; i<aDom.length ; i++ )
		{
			...
			else if ( cOption == 'p' && features.bPaginate )
			{
				...
				/*Add a div bottom black line for pageinfo  ---  liyang*/
		        insert.append("<div class='dataTables_blackline'></div>");
		...
		/*Add for pageing hide and show ---  liyang*/

	   _chengpagelengthposition();

		$(".dataTables_wrapper").hover(function(){
		...
		$(window).resize(function(){
			var dwpr = $(".dataTables_wrapper");
			var dshi = $(".dataTables_scrollHeadInner");
			for(var i=0;i<dwpr.length;i++){
				dwpr.eq(i).width(dwpr.eq(i).parent().width());
				dshi.eq(i).width(dwpr.eq(i).parent().width());
			};
	...
	/*Add for pageing postion ---  liyang*/
	function _chengpagelengthposition(){
	         var pwid = $(".dataTables_paginate").last().width();
			 if(pwid==0){
				pwid=123
			 }
			//$(".dataTables_length").css("right",pwid+20);
	}
	...
	function _fnInitialise ( settings )
	{
		...
		_fnAddOptionsHtml( settings );//加载除table之外的附件
	...
	function _fnFeatureHtmlProcessing ( settings )
	{
		return $('<div/>', {
			...
			.html("<div class=\"dataTables_processing_words\">"+settings.oLanguage.sProcessing+"</div>")
	...
	function _fnFeatureHtmlTable ( settings )
	{
		...
		var headerCloneId =$( table[0].cloneNode(false) ).attr('id')+'th';
	...
	function _fnScrollDraw ( settings )
	{
		...
		$.each( _fnGetUniqueThs( settings, headerCopy ), function ( i, el ) {
			...
			//liyang for strag head to chenge nth width bug
			el.style.width = settings.aoColumns[idx].nTh.offsetWidth-20;
		} );
		if ( footer ) {
	...
	DataTable.defaults = {
		...
		"oLanguage": {
			 ...
			 *    } );
			 */
			"sInfo": "当前_START_ - _END_条 , 共_TOTAL_条",
...
//liyang add plugs for th drag cheng bar width
...
(function($, window) {
	...
	ResizableColumns = (function() {
		...
		function ResizableColumns($table, options) {
			...
			this.$tablehideth = $table;

			this.$table = $("#" + $table.attr('id') + "th"); //get clone  table  th
		...
		ResizableColumns.prototype.createHandles = function() {
			...
			if (this.$table.find('tr th').length > 0) { //if have th
		...
		ResizableColumns.prototype.mousedown = function(e) {
			var $currentGrip, $leftColumn, $rightColumn, idx, leftColumnStartWidth, rightColumnStartWidth, _this = this;
			...
			$leftColumn2 = this.$tablehideth.find('tr th').eq(idx); //add for secend table leftcolumn

			$rightColumn2 = this.$tablehideth.find('tr th').eq(idx + 1); //add for secend table rightcolumn
			...
			$(document).on('mousemove.rc', function(e) {
				...
				$leftColumn2.width(newLeftColumnWidth); //add for secend table leftcolumn chenge width
				$rightColumn2.width(newRightColumnWidth); //add for secend table rightcolumn chenge width
...
function setMoveThBar(obj){

	setTimeout(function(){
			 	$("#"+obj.attr("id")).resizableColumns({
                	store: store
             	});
	},50)

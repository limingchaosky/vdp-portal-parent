            ...
            if (this.dom.resize) {
                ...
                if ($('div.dataTables_scrollBody').length) {
                    ...
                    if ($('div.dataTables_scrollBody').length > 0) {
                        ...
                        for (currentColumnIndex = -1; currentColumnIndex < this.s.dt.aoColumns.length - 1 ; currentColumnIndex++) {
						...
						//liyang take head table width equel body table width on mousemove
						var findtableid = $(this.s.dt.nTableWrapper).find("div.dataTables_scrollBody table").attr("id");

						//$("#"+findtableid).width($("#"+findtableid+"th").width());

						//$("#"+findtableid+" th").eq(nTh.index()).width(nTh.width());

						var findtableBody = $(this.s.dt.nTableWrapper).find("div.dataTables_scrollBody");

						var findtableHead = $(this.s.dt.nTableWrapper).find("div.dataTables_scrollHead");

						findtableBody.scroll(function(){
				            findtableHead.css("overflow","auto");
				            findtableHead.scrollLeft(findtableBody.scrollLeft());
			            });
...
*/
				...
				//liyang for  obj_wrapper don't chenge width , when strag end and mouseup
                //Save the state
                this.s.dt.oInstance.oApi._fnSaveState(this.s.dt);
            }
            ///////////////////////////////////////////////////////
